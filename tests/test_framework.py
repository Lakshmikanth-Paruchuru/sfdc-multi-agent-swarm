...
def test_hygiene():
    ...
    # Split employer-specific tokens so the literal word never appears in source.
    banned = (
        "nextgen2",
        "nextgen",
        "servicetitan",
    ...
    for path in ROOT.rglob("*"):
        if ".git" in path.parts or "tests" in path.parts or ".demo-sfdx" in path.parts or path.suffix in {".png", ".jpg", ".zip", ".pyc"}:
            continue
        if path.is_dir():
            if path.name == "{codebase,connected,sfdc,project,skills}":
                hits.append(str(path))
            continue
        if path.suffix not in {".py", ".md", ".mdc", ".json", ".sh", ".plist", ".html", ".js", ".yml", ".yaml", ".txt", ".command"}:
            continue
        try:
...
def test_templates():
    ...
    for folder in [
        ...
        _proj = "NEXT" + "GEN2"
        if inst.is_file() and _proj not in inst.read_text() and "/Users/" not in inst.read_text():
            ok(f"agency instructions {folder}")
        elif not inst.is_file():
            bad(f"agency instructions {folder}", "missing")
        else:
            bad(f"agency instructions {folder}", "contains absolute path or private project token")
